#!/usr/bin/env python3
"""
ClickUp Email Marketing Template Automation
Erstellt Email Marketing Liste mit allen Tasks für neue Kunden
"""
...
# Email Marketing Template Task IDs (aus dem Process Hub)
EMAIL_MARKETING_TEMPLATE_TASKS = [
    ("86c8bczb1", "📧 Kickoff & Zugänge sichern"),
    ("86c8bczf4", "🔧 Account Setup & Segmentierung"),
    ("86c8bczjr", "🔄 Flow-Strategie & Setup"),
    ("86c8bczne", "🎨 Template Design & Branding"),
    ("86c8bczrf", "🚀 Erste Campaign Launch"),
    ("86c8bczw1", "📅 Weekly Newsletter Prozess"),
    ("86c8bczzg", "📊 Monthly Reporting & Optimierung"),
]
...
def setup_email_marketing_for_client(client, space_id, client_name):
    """
    Erstellt Email Marketing Setup für einen neuen Kunden:
    1. Folder erstellen (falls nicht existiert)
    2. Liste "Email Marketing" erstellen
    3. Alle 7 Template-Tasks kopieren
    """
    print(f"\n📧 Email Marketing Setup für: {client_name}")
    print("=" * 60)
    
    # 1. Folder erstellen (oder vorhandenen nutzen)
    folder_id = None
    try:
        folder = client.create_folder(space_id, client_name)
        folder_id = folder['id']
        print(f"📁 Folder erstellt: {folder['name']} (ID: {folder_id})")
    except Exception as e:
        # Folder existiert wahrscheinlich schon - versuche zu finden
        print(f"⚠️  Folder existiert wahrscheinlich schon, suche...")
        try:
            # Hole alle Folders im Space
            import urllib.request
            req = urllib.request.Request(
            ...
            with urllib.request.urlopen(req, context=ctx) as response:
                ...
                for f in data.get('folders', []):
                    if f['name'] == client_name:
                        folder_id = f['id']
                        print(f"📁 Existierenden Folder gefunden: {f['name']} (ID: {folder_id})")
                        break
        except Exception as e2:
            print(f"❌ Konnte Folder nicht finden: {e2}")
            return None
    
    if not folder_id:
        print("❌ Kein Folder verfügbar")
        return None
    ...
    list_obj = client.create_list(
        ...
        content=f"Email Marketing Setup für {client_name}"
    )
    print(f"📋 Liste erstellt: {list_obj['name']} (ID: {list_obj['id']})")
    list_id = list_obj['id']
    
    # 3. Alle Template-Tasks kopieren
    print(f"\n📥 Kopiere {len(EMAIL_MARKETING_TEMPLATE_TASKS)} Tasks aus Template...")
    created_tasks = []
    for task_id, task_name in EMAIL_MARKETING_TEMPLATE_TASKS:
        try:
...
def main():
    parser = argparse.ArgumentParser(description="Email Marketing Template Setup für ClickUp Kunden")
    ...
    if args.dry_run:
        ...
        print(f"   Es würden {len(EMAIL_MARKETING_TEMPLATE_TASKS)} Tasks kopiert werden:")
        for _, name in EMAIL_MARKETING_TEMPLATE_TASKS:
            print(f"     - {name}")
        return
    
    client = ClickUpClient()
    result = setup_email_marketing_for_client(client, args.space_id, args.client_name)
    
    if result:
        print(f"\n🎉 Email Marketing Setup komplett für {args.client_name}!")
    else:
