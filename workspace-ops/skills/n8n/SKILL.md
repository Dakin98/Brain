...
## Extended Features (adsdrop Stack)

### Webhook Management

⚠️ **Known Issue:** Production webhooks don't reliably register via API - only formTrigger works consistently.

#### List Webhook Workflows
```bash
python3 scripts/webhook_manager.py --list
```

#### Check Webhook Status
```bash
python3 scripts/webhook_manager.py --check --workflow-id <workflow-id>
```

#### Fix Webhook Registration
Deactivate and reactivate workflow to force webhook registration:
```bash
python3 scripts/webhook_manager.py --fix --workflow-id <workflow-id>
```

#### Fix All Webhooks
```bash
python3 scripts/webhook_manager.py --fix-all
```
...
```bash
python3 scripts/error_recovery.py --list-failed --hours 24
```

#### Retry Specific Execution
```bash
python3 scripts/error_recovery.py --retry --execution-id <execution-id>
```

#### Auto-Retry Failed Workflows
Automatically retry recent failures (with retry tracking):
```bash
python3 scripts/error_recovery.py --auto-retry --workflow-id <workflow-id> --max-retries 3
```

#### Monitor Mode
Continuous monitoring with alerts:
```bash
python3 scripts/error_recovery.py --monitor --interval 300
```
...
#### Check All Workflows
```bash
python3 scripts/monitor_health.py --check-now
```

#### Daemon Mode (Continuous)
```bash
python3 scripts/monitor_health.py --daemon --interval 300
```

#### Check Specific Workflow
```bash
python3 scripts/monitor_health.py --check-now --workflow-id <id>
```
...
Available templates:
- **closing_flow** — Creates deal in Airtable + sends DocuSeal contract
- **onboarding_flow** — Processes onboarding + creates Drive folder + sends confirmation

#### Apply Template
```bash
python3 scripts/apply_template.py \
  --template closing \
  --name "Kunde XYZ" \
  --airtable-record recXXX \
  --activate
```
...
```bash
python3 scripts/export_docs.py --workflow-id <id> --output docs.md
```

#### Export All Workflows
```bash
python3 scripts/export_docs.py --all --output-dir ./n8n-docs
```
...
#### Check Specific Workflow
```bash
python3 scripts/check_env.py --workflow-id <id>
```

#### Check All Workflows
```bash
python3 scripts/check_env.py --all
```

#### Show Only Missing
```bash
python3 scripts/check_env.py --all --missing-only
```
...
5. Fix webhooks: `python3 scripts/webhook_manager.py --fix --workflow-id <id>`
6. Check environment: `python3 scripts/check_env.py --workflow-id <id>`
